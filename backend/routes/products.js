...
router.get('/reset', async (req, res) => {
  try {
    console.log('🔄 Manual reset triggered. Cleaning up...');
...
/**
 * GET /api/products
 * Self-healing route that seeds products if missing.
 */
router.get('/', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM products ORDER BY id ASC');
    if (result.rows.length < 16) {
      // Auto-trigger the reset logic if count is wrong
      console.log('🌱 Auto-correcting product count...');
      await pool.query('TRUNCATE TABLE products CASCADE');
      await pool.query(`INSERT INTO products (name, price, image_url, description) VALUES
        ('Coastal Collection - ocean whisper', 249.00, '/images/product_2.jpg', 'product is so gooood'),
        ('Coastal Collection - Obsidian shore', 249.00, '/images/product_3.jpg', 'product is so gooood'),
        ('Coastal Collection - velvet Tide', 249.00, '/images/product_4.jpg', 'product is so gooood'),
        ('Forever Collection - forever bloom', 899.00, '/images/product_5.jpg', 'product is so gooood'),
        ('Forever Collection - evergreen Promise', 1299.00, '/images/product_6.jpg', 'product is so gooood'),
        ('Charm Collection - Petal drop silver', 349.00, '/images/product_7.jpg', 'product is so gooood'),
        ('Charm Collection - Petal drop Gold', 349.00, '/images/product_8.jpg', 'product is so gooood'),
        ('Charm Collection - Evil eye Pendant Gold', 374.00, '/images/product_9.jpg', 'product is so gooood'),
        ('Initial Collection - Duo initials', 349.00, '/images/product_10.jpg', 'product is so gooood'),
        ('Chrono Collection - Aqua chrona', 599.00, '/images/product_11.jpg', 'product is so gooood'),
        ('Forever Collection - crystal bloom', 2499.00, '/images/product_12.jpg', 'product is so gooood'),
        ('Chrono Collection - obsidian hour', 1199.00, '/images/product_13.jpg', 'product is so gooood'),
        ('Luxe Pot collection - Opaline Luxe 1', 299.00, '/images/product_14.jpg', 'product is so gooood'),
        ('Luxe Pot collection - Opaline Luxe 2', 399.00, '/images/product_15.jpg', 'product is so gooood'),
        ('Luxe Pot collection - Opaline Luxe 3', 499.00, '/images/product_16.jpg', 'product is so gooood'),
        ('Luxe Pot collection - Opaline Luxe 4', 359.00, '/images/product_1.jpg', 'product is so gooood')
      `);
